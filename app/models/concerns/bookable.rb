module Bookable
  ...
  def overlaps
    overlapping_bookings = [ 
      garage_spot.bookings.end_during(start_time, end_time),
      garage_spot.bookings.start_during(start_time, end_time),
      garage_spot.bookings.happening_during(start_time, end_time),
      garage_spot.bookings.enveloping(start_time, end_time),
      garage_spot.bookings.identical(start_time, end_time)
    ].flatten
  ...
  def start_date_cannot_be_in_the_past
    if start_time && start_time < DateTime.now  # + (10.seconds)
      errors.add(:start_time, 'must be at least 1 minutes from present time')
    end
  end
