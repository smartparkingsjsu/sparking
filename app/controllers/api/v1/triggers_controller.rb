class API::V1::TriggersController < ApplicationController
  ...
  def create
    ...
    if @get_garage_spot.length >= @position.to_i
      @get_garage_spot = @get_garage_spot[@position.to_i-1]
      ...
      unless @get_license_plates.include? @license
        Notification.create(recipient_id: @get_garage_id, booking_id: @get_booking.id, confidence: @confidence, action: "License plate mismatch expecting " + @get_license_plates.to_s)
