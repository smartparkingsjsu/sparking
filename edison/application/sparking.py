...
def main():
    ...
    while(True):
        ...
        trigger = None
        print("Enter a number from 1-3 to trigger its camera")
        # trigger = raw_input()
        trigger = input()

        # Takes a picture
        if trigger is not None and (1 <= int(trigger) <= 3):
            # skip for testing
            ...
            response = alpr_client.getLicensePlate(edison.sensors["camera"].getImagePathSplit(int(trigger)))
